...
class DataFrameStore(DataStore):
    def __init__(self, path: str, compute: Optional[Callable[[], DataFrame]] = None) -> None:
        self.path = path
        if compute:
...
class HdfStore(DataStore):
    ...
    def __init__(self, path: str, table: str, compute: Optional[Callable[[], DataFrame]] = None) -> None:
        self.table = table
        if compute:
    ...
    # This typevar ends up invisible to flake8, so we need to #NOQA F821 the def below. Can be removed once
    # https://github.com/PyCQA/pyflakes/issues/441 becomes available via pip.

    def _mangle_where_in_query(self, query: T) -> T:  # NOQA: F821
        # See: https://github.com/PyTables/PyTables/issues/638
