...
class EURealizer():
    ...
    def _comparisons(self, slot, template, what_type, added_slots, idx):
        if slot.fact.what < 0:
            template.add_slot(idx-1, LiteralSlot(self.dic.COMPARISONS.get('less')))
            added_slots += 1
            idx += 1
        else:
            template.add_slot(idx-1, LiteralSlot(self.dic.COMPARISONS.get('more')))
            added_slots += 1
            idx += 1
        if "eu" in what_type:
            template.add_slot(idx-1, LiteralSlot(self.dic.COMPARISONS.get('eu')))
            added_slots += 1
            idx += 1
        elif "us" in what_type:
            template.add_slot(idx-1, LiteralSlot(self.dic.COMPARISONS.get('us')))
            added_slots += 1
            idx += 1
        elif "similar" in what_type:
            template.add_slot(idx-1, LiteralSlot(self.dic.COMPARISONS.get('similar')))
            added_slots += 1
            idx += 1
    ...
    def _unit_rank(self, slot):
        ...
        if not change:
            ...
            if grouped_by == '_time':
                template.add_slot(idx, LiteralSlot(self.dic.COMPARISONS.get('rank')))
            else:   
    ...
    def _time_month(self, random, slot):
        ...
        if (slot.attributes['name_type'] in ['full', 'short']) or (
            ...
            self._update_slot_value(slot, self.dic.TEMPLATES.get('month').format(self.dic.MONTHS.get(month), year))
    ...
    def _time_change_year(self, random, slot):
        ...
        if slot.attributes['name_type'] == 'full':
            self._update_slot_value(slot, self.dic.TEMPLATES.get('year_change').format(match.group(1), match.grouo(2)))
    ...
    def _time_change_month(self, random, slot):
        ...
        idx = template.components.index(slot)
        if slot.attributes['name_type'] == 'full':
            self._update_slot_value(slot, self.dic.TEMPLATES.get('month_change').format(month1, year1, month2, year2))
        elif slot.attributes['name_type'] == 'short':
